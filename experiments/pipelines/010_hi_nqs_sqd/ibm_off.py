"""Pipeline 010 hi_nqs_sqd / ibm_off — force GPU fallback (no IBM solve_fermion).

Wraps :func:`qvartools.methods.nqs.run_hi_nqs_sqd` with the ``[ibm_off]``
section of ``configs/010_hi_nqs_sqd.yaml``.

Usage::

    python experiments/pipelines/010_hi_nqs_sqd/ibm_off.py h2 --device cuda
    python experiments/pipelines/010_hi_nqs_sqd/ibm_off.py h2 \
...
def main() -> None:
    ...
    parser = create_base_parser(
        f"Pipeline {PIPELINE_ID} {METHOD} ({VARIANT}): force GPU fallback (no IBM solve_fermion)."
    )
