"""Pipeline 013 nqs_skqd / default — two-stage NQS train + Krylov expand + diag.

Wraps :func:`qvartools.methods.nqs.run_nqs_skqd` with the ``[default]``
section of ``configs/013_nqs_skqd.yaml``.

Usage::

    python experiments/pipelines/013_nqs_skqd/default.py h2 --device cuda
    python experiments/pipelines/013_nqs_skqd/default.py h2 \
...
from qvartools.methods.nqs import NQSSKQDConfig, run_nqs_skqd  # noqa: E402
...
def main() -> None:
    ...
    parser = create_base_parser(
        f"Pipeline {PIPELINE_ID} {METHOD} ({VARIANT}): two-stage NQS train + Krylov expand + diag."
    )
    ...
    valid_keys = NQSSKQDConfig.__dataclass_fields__.keys()
    config_kwargs = {k: v for k, v in section.items() if k in valid_keys}
    config_kwargs["device"] = device
    config = NQSSKQDConfig(**config_kwargs)

    t_start = time.perf_counter()
    result = run_nqs_skqd(hamiltonian, mol_info, config=config)
