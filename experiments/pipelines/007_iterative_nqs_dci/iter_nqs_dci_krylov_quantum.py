"""Pipeline 07b: NF training + DCI merge -> Quantum Circuit Krylov.
...
Same as Group 02b but with the NF+DCI basis providing context.
"""
...
def main() -> None:
    ...
    parser = create_base_parser(
        "Pipeline 07b: NF + DCI merge -> Quantum Circuit Krylov.",
    )
    ...
    print(f"  NF stages time: {nf_time:.2f} s")
    print("-" * 60)
    ...
    print("\n" + "=" * 60)
    print("PIPELINE 07b: NF+DCI MERGE -> QUANTUM KRYLOV RESULTS")
    print("=" * 60)
    ...
    if result.metadata:
        ...
        if energies:
            ...
            for i, e in enumerate(energies):
                step_err = (
                    (e - exact_energy) * 1000.0 if exact_energy is not None else None
                )
                step_str = (
                    f"  (error: {step_err:.4f} mHa)" if step_err is not None else ""
                )
