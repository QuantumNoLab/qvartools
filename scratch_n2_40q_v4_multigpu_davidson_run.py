"""N2-40Q HF v4 multi-GPU Davidson benchmark.

Tests full multi-GPU stack: row-partitioned H Davidson + (optionally) multi-GPU PT2.
"""
import argparse
import json
import os
...
def main():
    ...
    parser.add_argument("--mode", required=True,
                        choices=["mgd_only", "mgd_and_mgpt2"],
                        help="mgd_only: multi-GPU Davidson only; "
                             "mgd_and_mgpt2: both Davidson and PT2 multi-GPU")
    ...
    print(f"[v4 multigpu-dv mode={args.mode}]", flush=True)
    print(f"  CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES','<all>')}  "
          f"visible={n_visible}", flush=True)
    for i in range(n_visible):
        print(f"    GPU {i}: {torch.cuda.get_device_name(i)}", flush=True)

    use_mgpt2 = (args.mode == "mgd_and_mgpt2")
    ...
    cfg = HINQSSQDv4Config(
        ...
        use_gpu_davidson=False,           # use multi-GPU instead
        use_multi_gpu_davidson=True,
        use_multi_gpu=use_mgpt2,
        **var,
    )
    ...
    print(f"\n{'='*90}")
    print(f"  FINAL v4-MGD ({args.variant} @ {args.budget}) mode={args.mode}")
    ...
    record = {
        ...
        "budget": args.budget,
        "seed": args.seed,
        "n_gpus_visible": int(n_visible),
        "use_multi_gpu_davidson": True,
        "use_multi_gpu_pt2": use_mgpt2,
        "variant_params": var,
        "n_samples": n_samples, "top_k": top_k, "max_basis_size": max_basis,
