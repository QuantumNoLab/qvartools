"""Pipeline 010 hi_nqs_sqd / default — baseline HI+NQS+SQD (auto IBM detect).

Wraps :func:`qvartools.methods.nqs.run_hi_nqs_sqd` via ``METHODS_REGISTRY`` with
the ``[default]`` section of ``configs/010_hi_nqs_sqd.yaml``.
...
def main() -> None:
    ...
    parser = create_base_parser(
        f"Pipeline {PIPELINE_ID} {METHOD_KEY} ({VARIANT}): baseline HI+NQS+SQD (auto IBM detect)."
    )
