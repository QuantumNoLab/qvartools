"""Pipeline 012 nqs_sqd / default — two-stage NQS train + SQD diag (no iteration).

Wraps :func:`qvartools.methods.nqs.run_nqs_sqd` with the ``[default]``
section of ``configs/012_nqs_sqd.yaml``.

Usage::

    python experiments/pipelines/012_nqs_sqd/default.py h2 --device cuda
    python experiments/pipelines/012_nqs_sqd/default.py h2 \
...
from qvartools.methods.nqs import NQSSQDConfig, run_nqs_sqd  # noqa: E402
...
def main() -> None:
    ...
    parser = create_base_parser(
        f"Pipeline {PIPELINE_ID} {METHOD} ({VARIANT}): two-stage NQS train + SQD diag (no iteration)."
    )
    ...
    valid_keys = NQSSQDConfig.__dataclass_fields__.keys()
    config_kwargs = {k: v for k, v in section.items() if k in valid_keys}
    config_kwargs["device"] = device
    config = NQSSQDConfig(**config_kwargs)

    t_start = time.perf_counter()
    result = run_nqs_sqd(hamiltonian, mol_info, config=config)
