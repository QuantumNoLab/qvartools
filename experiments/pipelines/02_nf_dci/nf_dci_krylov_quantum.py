"""Pipeline 02b: NF + Direct-CI merged basis -> Quantum Circuit Krylov.

Runs a hybrid pipeline:
  1. NF-NQS training (physics-guided mixed-objective)
  2. Diversity-aware basis extraction (merges NF + essential HF+S+D configs)
  3. Quantum circuit Krylov via QuantumCircuitSKQD (Trotterized evolution)

The NF training in stages 1-2 improves sampling quality. Stage 3 uses
QuantumCircuitSKQD which generates its own Krylov basis internally via
Trotterized time evolution and injects essential configs (HF+S+D)
independently.

Uses skip_nf_training=False for NF training, then quantum SKQD for
...
def main() -> None:
    ...
    parser = create_base_parser(
        "Pipeline 02b: NF + Direct-CI -> Quantum Circuit Krylov.",
    )
    ...
    # --- Stage 1-2: NF training + basis extraction (via FlowGuidedKrylovPipeline) ---
    ...
    # Stage 2: Basis extraction + diversity selection (merges NF + HF+S+D)
    basis = pipeline.extract_and_select_basis()
    nf_time = time.perf_counter() - t_start

    print(f"NF training complete: {n_epochs} epochs")
