#!/bin/bash
# Same 4-mode × 2-seed ablation but on N2-CAS(10,20) (40Q).
# Tests whether NQS failure is system-agnostic.
set -u
cd /home/leo07010/HI-VQE
source /home/leo07010/HI-VQE/scripts/setup_cuda_env.sh

HN=$(hostname)
echo "[diag-40q | $HN] start $(date)"

TASKS=(
    "baseline:42"      "baseline:777"
    "no_nqs:42"        "no_nqs:777"
    "nqs_only:42"      "nqs_only:777"
    "random_sample:42" "random_sample:777"
)

run_task() {
    local gpu=$1
    local t="${TASKS[$gpu]}"
    IFS=":" read -r mode seed <<< "$t"
    local tag="${mode}_s${seed}"
    echo "[diag-40q | GPU=$gpu | $tag] $(date)"
    export CUDA_VISIBLE_DEVICES=$gpu
    out="results/diagnose_nqs_40q/${tag}.json"
    log="logs/diagnose_nqs_40q/${tag}_${SLURM_JOB_ID}.log"
    python -u scratch_diagnose_nqs_role.py \
        --mode="$mode" --seed=$seed \
        --molecule="N2-CAS(10,20)" \
        --n_samples=500000 --top_k=20000 --budget=100000 \
        --max_iterations=8 --pt2_top_n=10000 \
        --out="$out" \
        > "$log" 2>&1 || echo "  [GPU $gpu $tag FAILED]"
    echo "[diag-40q | GPU=$gpu | $tag] done $(date)"
}

pids=()
for gpu in 0 1 2 3 4 5 6 7; do
    run_task $gpu &
    pids+=($!)
done

rc=0
for p in "${pids[@]}"; do
    if ! wait $p; then rc=1; fi
done
echo "[diag-40q] done $(date) rc=$rc"
exit $rc
