...
Pipeline:
  ...
  4. SQD batch diagonalization with noise injection and S-CORE
...
configurations.  The expanded basis is then replicated to simulate shot
sampling, optionally corrupted with noise, and diagonalized via SQD
with self-consistent configuration recovery (S-CORE).
"""
...
def main() -> None:
    ...
    n_unique_pre_pt2 = basis.shape[0]
    print(f"NF+DCI merged basis: {n_unique_pre_pt2} unique configs")
    ...
    n_unique_post_pt2 = basis.shape[0]
    print(
        f"PT2 expansion: {n_unique_pre_pt2} -> {n_unique_post_pt2} configs "
        f"(+{n_unique_post_pt2 - n_unique_pre_pt2})"
    )

    # --- Stage 3: SQD with noise + S-CORE ---
    # Replicate basis to simulate shot sampling
    shots_mult = get_shots_multiplier(n_unique_post_pt2, n_qubits)
    total_shots = n_unique_post_pt2 * shots_mult
    ...
    # SQD batch diagonalization with noise + S-CORE
    ...
    # --- Results summary ---
    print("\n" + "=" * 60)
    print("NF + DCI + PT2 -> SQD RESULTS (noise + S-CORE)")
    print("=" * 60)
