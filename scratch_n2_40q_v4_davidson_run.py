"""N2-40Q HF v4 GPU Davidson benchmark.

Compares scipy eigsh (CPU Davidson) vs torch-native Davidson (GPU matvec) on
the same v4 pipeline. Everything else identical.
"""
import argparse
import json
import time
...
def main():
    ...
    parser.add_argument("--gpu_davidson", action="store_true",
                        help="Use GPU Davidson (vs scipy eigsh)")
    ...
    mode = "gpu_davidson" if args.gpu_davidson else "scipy_eigsh"
    print(f"[v4 {mode}] Variant: {args.variant}  {var}", flush=True)
    ...
    cfg = HINQSSQDv4Config(
        ...
        use_multi_gpu=False,
        use_gpu_davidson=args.gpu_davidson,
        **var,
    )
    ...
    print(f"\n{'='*90}")
    print(f"  FINAL v4 [{mode}] ({args.variant} @ {args.budget})")
    ...
    record = {
        ...
        "budget": args.budget,
        "seed": args.seed,
        "use_gpu_davidson": bool(args.gpu_davidson),
        "variant_params": var,
        "n_samples": n_samples, "top_k": top_k, "max_basis_size": max_basis,
