"""Pipeline 011 hi_nqs_skqd / default — baseline HI+NQS+SKQD with Krylov expansion.

Wraps :func:`qvartools.methods.nqs.run_hi_nqs_skqd` with the ``[default]``
section of ``configs/011_hi_nqs_skqd.yaml``.

Usage::

    python experiments/pipelines/011_hi_nqs_skqd/default.py h2 --device cuda
    python experiments/pipelines/011_hi_nqs_skqd/default.py h2 \
...
def main() -> None:
    ...
    parser = create_base_parser(
        f"Pipeline {PIPELINE_ID} {METHOD} ({VARIANT}): baseline HI+NQS+SKQD with Krylov expansion."
    )
