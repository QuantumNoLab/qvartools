...
Wraps :func:`qvartools.methods.nqs.run_nqs_skqd` via ``METHODS_REGISTRY`` with
the ``[default]`` section of ``configs/013_nqs_skqd.yaml``.
...
def main() -> None:
    ...
    parser = create_base_parser(
        f"Pipeline {PIPELINE_ID} {METHOD_KEY} ({VARIANT}): two-stage NQS train + Krylov expand + diag."
    )
