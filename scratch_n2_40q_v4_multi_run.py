"""N2-40Q HF v4 multi-GPU test.

Same config as v4 single-GPU benchmark but with use_multi_gpu=True to split
per-iter PT2 scoring + final PT2 correction across all visible GPUs.

Expected vs single-GPU v4:
  - PT2-related phases (pt2 scoring + final_PT2): linear-ish speedup
  - sqd (Davidson): unchanged (not yet parallelized)
  - Overall: if pt2+final_PT2 was 35% of time, 8 GPUs gives ~7x on that part
             → overall ~1.4x total wall speedup (limited by sqd bottleneck)
"""
import argparse
import json
import os
...
VARIANTS = {
    "default": {"classical_expansion_top_n": 1000, "pt2_top_n": 5000},
    "small":   {"classical_expansion_top_n": 200,  "pt2_top_n": 5000},
    "big":     {"classical_expansion_top_n": 2000, "pt2_top_n": 10000},
}

BUDGETS = {
    "100k":  (1_000_000, 10_000,  100_000),
...
def main():
    ...
    print(f"[multi-GPU v4] CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', '<all>')}",
          flush=True)
    print(f"  visible GPUs: {n_visible}", flush=True)
    for i in range(n_visible):
        print(f"    GPU {i}: {torch.cuda.get_device_name(i)}", flush=True)
    print(f"Variant: {args.variant}  {var}", flush=True)
    print(f"Budget:  {args.budget}  samples={n_samples:,} top_k={top_k:,} max_basis={max_basis:,}",
          flush=True)
    ...
    print(f"\n{'='*90}")
    print(f"  FINAL v4-MULTI ({args.variant} @ {args.budget}) n_gpus={meta.get('v4_n_gpus_used',1)}")
    print(f"  iter0_err   = {iter0_err:+.3f} mHa" if iter0_err is not None else "")
    ...
    record = {
        ...
        "budget": args.budget,
        "seed": args.seed,
        "n_gpus_used": int(meta.get("v4_n_gpus_used", 1)),
        "multi_gpu_active": bool(meta.get("v4_multi_gpu", False)),
        "variant_params": var,
        "n_samples": n_samples, "top_k": top_k, "max_basis_size": max_basis,
