...
Wraps :func:`qvartools.methods.nqs.run_nqs_sqd` via ``METHODS_REGISTRY`` with
the ``[default]`` section of ``configs/012_nqs_sqd.yaml``.
...
def main() -> None:
    ...
    parser = create_base_parser(
        f"Pipeline {PIPELINE_ID} {METHOD_KEY} ({VARIANT}): two-stage NQS train + SQD diag (no iteration)."
    )
