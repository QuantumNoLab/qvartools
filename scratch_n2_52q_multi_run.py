"""52Q v4 with combined top_n × multi-temperature sampling scan.

Tests two purely NQS-driven mechanisms simultaneously:
  - classical_expansion_top_n (2000 → 20000): more diverse expansion seeds
  - multi-temperature sampling: union of samples at K different T's

Both are pure NQS / NQS+expansion improvements (no CIPSI mechanics).
"""
...
def main():
    ...
    p.add_argument("--top_n", type=int, required=True)
    p.add_argument("--temps", type=str, default="",
                   help='Comma-separated T values, e.g. "0.3,1.0,2.0"; empty = single annealed T')
    p.add_argument("--n_samples", type=int, default=2_000_000)
    ...
    if args.temps:
        try:
            multi_T = tuple(float(t) for t in args.temps.split(","))
        except Exception:
            multi_T = None

    print(f"[52Q multi scan] top_n={args.top_n} temps={multi_T} "
          f"top_k={args.top_k} budget={args.budget}", flush=True)
    ...
    print(f"\n=== top_n={args.top_n} temps={multi_T} ===")
    ...
    record = {
        "system": "N2-CAS(10,26)",
        "top_n": args.top_n, "temps": list(multi_T) if multi_T else None,
        "n_samples": args.n_samples, "top_k": args.top_k,
        "budget": args.budget, "pt2_top_n": args.pt2_top_n,
