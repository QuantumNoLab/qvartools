"""Pipeline 08a: NF training + DCI merge + PT2 expansion -> Iterative NQS + Krylov.
...
Same as Group 03 for stages 1-2.5, then Group 06 iterative NQS for stage 3.
"""
...
def main() -> None:
    ...
    parser = create_base_parser(
        "Pipeline 08a: NF + DCI + PT2 -> Iterative NQS + Krylov.",
    )
    ...
    print(
        f"  PT2 expansion: {basis_before} -> {basis.shape[0]} configs (+{basis.shape[0] - basis_before})"
    )

    nf_pt2_time = time.perf_counter() - t_start

    # === Stage 3: Iterative NQS + Krylov ===
    ...
    print("\n" + "=" * 60)
    print("PIPELINE 08a: NF+DCI+PT2 -> ITERATIVE NQS+KRYLOV RESULTS")
    print("=" * 60)
    print(f"NF training      : {n_epochs} epochs")
